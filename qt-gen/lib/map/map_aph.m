function APH = map_aph(e1,e2,e3,nmax) 
% APH = map_aph(e1,e2,e3,nmax) - moment matching of APH(n) distribution
% nmax = max order (default: 10)
% Technique published in A.Bobbio, A.Horvath, M.Telek, Matching three moments
...
elseif n2 > n/(n-1) && n3 > un_1 % case 2 of 2
    ...
    K22 = sqrt(3*K4^2/(4*K3^2) - 3*K5/(K1*K3*n2) + sqrt(3*K21^2 - n*K2/(n2*K1^2*K3)));
    if n2 > un_1 && n3 < 3*n2/2
        f = K13+K15-K17;
    elseif n3 == 2*n2/2
        f = K19;
    ...
    error('moment set cannot be matched with an APH distribution')
end
